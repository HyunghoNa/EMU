...
class DMAQ_QattenMixer(nn.Module):
    ...
    def forward(self, agent_qs, states, actions=None, max_q_i=None, is_v=False, obs=None):
        bs = agent_qs.size(0)

        w_final, v, attend_mag_regs, head_entropies = self.attention_weight(agent_qs, states, actions, obs=obs) #obs=obs, modified for CDS @ grf
