...
class Qatten_Weight(nn.Module):
    ...
    def forward(self, agent_qs, states, actions, obs=None):
        states = states.reshape(-1, self.state_dim)
        #if 'academy' in  self.args.env and 'cds' in self.args.learner:
        if 'academy' in  self.args.env :
            unit_states = obs.reshape(-1, obs.shape[2], obs.shape[3])            
        else:
